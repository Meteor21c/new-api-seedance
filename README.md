...
| `RELAY_RESPONSE_HEADER_TIMEOUT` | How long the relay waits for upstream response headers, seconds; set `0` to disable. Only bounds the header wait; streaming after the headers arrive is unaffected. Non-streaming upstreams usually send headers after generation finishes, so leave headroom | `1800` |
