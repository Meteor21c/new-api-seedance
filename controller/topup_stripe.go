...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	stripeChargeAmount := getStripeChargeAmount(float64(req.Amount), user.Group)

	reference := fmt.Sprintf("new-api-ref-%d-%d-%s", user.Id, time.Now().UnixMilli(), randstr.String(4))
	referenceId := "ref_" + common.Sha1([]byte(reference))

	payLink, err := genStripeLink(referenceId, user.StripeCustomer, user.Email, req.Amount, stripeChargeAmount, req.SuccessURL, req.CancelURL)
	if err != nil {
...
//   - amount: quantity of units to purchase
	//   - stripeChargeAmount: final Stripe-unit amount after group and tier pricing
//   - successURL: custom URL to redirect after successful payment (empty for default)
//   - cancelURL: custom URL to redirect when payment is canceled (empty for default)
//
// Returns the checkout session URL or an error if the session creation fails.
func genStripeLink(referenceId string, customerId string, email string, amount int64, stripeChargeAmount float64, successURL string, cancelURL string) (string, error) {
	...
	baseStripeAmount := float64(amount)
	if operation_setting.GetQuotaDisplayType() == operation_setting.QuotaDisplayTypeTokens {
		baseStripeAmount /= common.QuotaPerUnit
	}
	if math.Abs(stripeChargeAmount-baseStripeAmount) < 0.000001 {
		lineItem.Price = stripe.String(setting.StripePriceId)
		lineItem.Quantity = stripe.Int64(int64(math.Round(baseStripeAmount)))
	} else {
		// A configured Price ID can only charge its fixed unit price. Use an
		// inline one-time price whenever a group ratio, tier multiplier, or
		// token-to-USD conversion changes the checkout amount.
		unitAmount := int64(math.Round(stripeChargeAmount * setting.StripeUnitPrice * 100))
		if unitAmount <= 0 {
			return "", fmt.Errorf("无效的 Stripe 支付金额")
		}
		lineItem.PriceData = &stripe.CheckoutSessionLineItemPriceDataParams{
			Currency:   stripe.String("usd"),
			UnitAmount: stripe.Int64(unitAmount),
			ProductData: &stripe.CheckoutSessionLineItemPriceDataProductDataParams{
				Name: stripe.String("New API recharge"),
			},
		}
...
func getStripePayMoney(amount float64, group string) float64 {
	return getStripeChargeAmount(amount, group) * setting.StripeUnitPrice
}
